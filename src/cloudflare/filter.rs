...
struct Filter_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<FilterData>,
}

#[derive(Clone)]
pub struct Filter(Rc<Filter_>);
...
impl Filter {
    ...
    pub fn set_ref(self, v: impl Into<PrimField<String>>) -> Self {
        self.0.data.borrow_mut().ref_ = Some(v.into());
        self
    }
    ...
    pub fn expression(&self) -> PrimExpr<String> {
        PrimExpr::new(self.shared().clone(), format!("{}.expression", self.extract_ref()))
    }
...
impl ToListMappable for Filter {
    type O = ListRef<FilterRef>;
...
impl Resource_ for Filter_ {
    fn extract_resource_type(&self) -> String {
        "cloudflare_filter".into()
    }
...
pub struct BuildFilter {
    pub tf_id: String,
    #[doc= "The filter expression to be used."]
...
impl BuildFilter {
    pub fn build(self, stack: &mut Stack) -> Filter {
        let out = Filter(Rc::new(Filter_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(FilterData {
